...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
    ...
    const prefix = (threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX;
     if (args.length == 0) return api.sendMessage(`Bạn có thể dùng:\n\n${prefix}${this.config.name} user => nó sẽ lấy thông tin những người dùng cao nhất của bot.\n\n${prefix}${this.config.name} thread => nó sẽ lấy thông tin những nhóm cao nhất của bot.\n\n${prefix}${this.config.name} money => nó sẽ lấy thông tin số người dùng giàu nhất trong data bot.`, event.threadID, event.messageID);
